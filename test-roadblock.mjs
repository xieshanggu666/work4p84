...
// 贴边路线：封闭区边界同样属于封闭范围，必须被拦截（回归：曾漏判导致派发未被拦截）
assert(pointInPolygon([0.5, 1], square), '上沿边界上的点视为区内')
assert(pointInPolygon([1, 0.5], square), '右沿边界上的点视为区内')
assert(pointInPolygon([0, 0], square), '多边形顶点视为区内')
assert(!pointInPolygon([1.0001, 0.5], square), '贴近但在界外的点仍视为区外')
assert(pathBlocked([[0.3, 1], [0.7, 1]], square), '贴着上沿的共线路段被拦截')
assert(pathBlocked([[1, 0.3], [1, 0.7]], square), '贴着右沿的共线路段被拦截')
assert(pathBlocked([[0.2, 1], [0.8, 1]], square), '起终点均落在边界上的运输路线被拦截')
assert(!pathBlocked([[-1, 1.0001], [2, 1.0001]], square), '贴近但未接触边界的路线仍放行')
